    ...
    public class FPD_PropertiesFoldout : PropertyDrawer
    {
        ...
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            ...
            if (frameStyle == null)
            {
                ...
                else if (Attribute.frameStyleID == 1) frameStyle = FGUIResources.BGInBoxStyle;
                else if (Attribute.frameStyleID == 2) frameStyle = FGUIResources.HeaderBoxStyle;
                else if (Attribute.frameStyleID == 3) frameStyle = FGUIResources.FrameBoxStyle;
                else if (Attribute.frameStyleID == 4) frameStyle = FGUIResources.FoldStyle;
                else if (Attribute.frameStyleID == 5) frameStyle = FGUIResources.ViewBoxStyle;
                else if (Attribute.frameStyleID == 6) frameStyle = FGUIResources.BGInBoxBlankStyle;

                if (frameStyle == null) if (!string.IsNullOrWhiteSpace(Attribute.frameStyle)) frameStyle = GUI.skin.GetStyle(Attribute.frameStyle);
                if (frameStyle == null) frameStyle = FGUIResources.BGInBoxStyle;
            }
