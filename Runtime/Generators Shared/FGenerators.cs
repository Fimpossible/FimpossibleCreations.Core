            ...
            if (foldout)
            {
                ...
                if (toDraw.Count > 0)
                    for (int i = 0; i < toDraw.Count; i++)
                    {
                        ...
                        if (moveButtons)
                        {
                            if (i > 0) if (GUILayout.Button(new GUIContent(FGUIResources.Tex_ArrowUp, "Move this element to be executed before above one"), GUILayout.Width(24))) { T temp = toDraw[i - 1]; toDraw[i - 1] = toDraw[i]; toDraw[i] = temp; }
                            if (i < toDraw.Count - 1) if (GUILayout.Button(new GUIContent(FGUIResources.Tex_ArrowDown, "Move this element to be executed after below one"), GUILayout.Width(24))) { T temp = toDraw[i + 1]; toDraw[i + 1] = toDraw[i]; toDraw[i] = temp; }
                        }
...
return 1f;
    ...
    public class PrefabReference
    {
        ...
        public static void DrawPrefabsList<T>(List<T> list, ref bool foldout, ref int selected, ref bool thumbnails, Color defaultC, Color selectedC, float viewWidth = 500f, int previewSize = 72, bool searchButtons = false, UnityEngine.Object toDirty = null, bool allowAdding = true) where T : PrefabReference, new()
        {
            ...
            EditorGUILayout.BeginVertical(FGUIResources.BGInBoxBlankStyle);
            EditorGUILayout.BeginHorizontal();

            string f = FGUIResources.GetFoldSimbol(foldout);
            if (GUILayout.Button(f + "  Prefabs (" + list.Count + ")", FGUIResources.FoldStyle, GUILayout.Height(20))) foldout = !foldout;
            ...
            if (foldout)
            {
                ...
                if (searchButtons)
                {
                    if (GUILayout.Button(new GUIContent(FGUIResources.Tex_Rename), new GUILayoutOption[] { GUILayout.MaxWidth(24), GUILayout.Height(20) }))
                        thumbnails = !thumbnails;

                    if (list.Count > 0 && list[0] != null && list[0].Prefab != null)
                    {
                        if (GUILayout.Button(new GUIContent(FGUIResources.Tex_SearchNumeric, "Search for prefabs with the same name but with numbers"), new GUILayoutOption[] { GUILayout.MaxWidth(48), GUILayout.Height(20) }))
                        {
                        ...
                        if (GUILayout.Button(new GUIContent(FGUIResources.Tex_SearchDirectory, "Get all prefabs inside it's directory"), new GUILayoutOption[] { GUILayout.MaxWidth(48), GUILayout.Height(20) }))
                        {
                            List<GameObject> pfs = new List<GameObject>();
                            pfs.Add(list[0].Prefab);
                    ...
                    if (list.Count > 0)
                    {
                        ...
                        if (GUILayout.Button(new GUIContent(FGUIResources.Tex_Remove, "Remove all prefabs from list"), FGUIResources.ButtonStyle, new GUILayoutOption[] { GUILayout.MaxWidth(24), GUILayout.Height(19) }))
                            list.Clear();
                ...
                if (allowAdding)
                {
                    ...
                    GUIStyle d = FGUIInspector.Style(new RectOffset(0, 0, 0, 0), new RectOffset(0, 0, 0, 0), new Color(0.2f, 0.7f, 0.3f, 0.5f), Vector4.zero, 0);
                    d = new GUIStyle(d) { alignment = TextAnchor.MiddleCenter };

                    GUI.Box(drop, new GUIContent(FGUIResources.Tex_Drag, "Drag & Drop prefabs here"), d);
