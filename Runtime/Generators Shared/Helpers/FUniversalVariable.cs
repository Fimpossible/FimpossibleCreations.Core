...
namespace FIMSpace.FGenerating
{
    ...
    public class FUniversalVariable
    {
        ...
        public Vector2Int GetVector2Int() { return new Vector2(_value.x, _value.y).V2toV2Int(); }
        public Vector3 GetVector3() { return new Vector3(_value.x, _value.y, _value.z); }
        public Vector3Int GetVector3Int() { return new Vector3(_value.x, _value.y, _value.z).V3toV3Int(); }
        ...
        public virtual bool Editor_DisplayVariableGUI( GUILayoutOption[] guiLayoutOptions = null)
        {
            ...
            {
                if (IsFloat) _value = EditorGUILayout.Vector2Field(nameG, _value, options ); else SetValue( EditorGUILayout.Vector2IntField(nameG, GetVector2().V2toV2Int(), options ) );
            }
            else if (GetVariableType() == (int)EVariableType.Vector3)
            {
                if (IsFloat) _value = EditorGUILayout.Vector3Field(nameG, _value, options ); else SetValue(EditorGUILayout.Vector3IntField(nameG, GetVector3().V3toV3Int(), options ) );
            }
