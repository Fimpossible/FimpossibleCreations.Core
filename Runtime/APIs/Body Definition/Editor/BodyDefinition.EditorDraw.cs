    ...
    public static partial class BodyDefinitionEditorDraw
    {
        ...
        static void DrawTopAutoButtons(SerializedProperty sp, BodyDefinition definition)
        {
            ...
            if (definition.Animator)
                ...
                {
                    GUILayout.Space(8);
                    if (GUILayout.Button(new GUIContent("  Auto-Get Humanoid Bones", FGUIResources.FindIcon("SPR_BodyDefinition")), FGUIResources.ButtonStyle, GUILayout.Height(22))) { definition.AutoSetUsingHumanoidReferences(); EditorUtility.SetDirty(sp.serializedObject.targetObject); }
                    return;
                }
            ...
            if (definition.BaseTransform || definition.Animator)
            {
                ...
                if (GUILayout.Button(new GUIContent("  Try Auto-Find Generic Bones", FGUIResources.Tex_Bone), FGUIResources.ButtonStyle, GUILayout.Height(22))) 
                { 
        ...
        static void DrawCategoryBar(BodyDefinition definition)
        {
            ...
            if (GUILayout.Button(FGUIResources.GetFoldSimbol((int)definition._Editor_BodyCategory != -1, true), EditorStyles.label, bHeight, GUILayout.Width(18)))
            {
            ...
            if (GUILayout.Button(new GUIContent(definition._Editor_DrawGizmos ? FGUIResources.Tex_Gizmos : FGUIResources.Tex_GizmosOff, "Click to switch gizmos draw on the scene"), EditorStyles.label, GUILayout.Width(18), bHeight))
            {
        ...
        static void DrawCategoryHeader(EBodyCategory category)
        {
            ...
            EditorGUILayout.LabelField(category.ToString() + " References", FGUIResources.HeaderStyle);
