...
{
    public static class FGUIInspector
    {
        ...
        {
            if (frame) EditorGUILayout.BeginHorizontal(FGUIResources.HeaderBoxStyle); else EditorGUILayout.BeginHorizontal();
            ...
            if (GUILayout.Button(f + "     " + title + "     " + f, big ? FGUIResources.HeaderStyleBig : FGUIResources.HeaderStyle, GUILayout.Height(height))) foldout = !foldout;
            if (icon != null) if (GUILayout.Button(new GUIContent(icon), EditorStyles.label, new GUILayoutOption[2] { GUILayout.Width(iconsSize), GUILayout.Height(iconsSize) })) { foldout = !foldout; }
            GUILayout.Label(new GUIContent(" "), GUILayout.Width(1));
        ...
        public static void HeaderBox(string title, bool frame, Texture icon = null, int height = 20, int iconsSize = 19, bool big = false)
        {
            ...
            if (GUILayout.Button(title, big ? FGUIResources.HeaderStyleBig : FGUIResources.HeaderStyle, GUILayout.Height(height))) { }
            if (icon != null) if (GUILayout.Button(new GUIContent(icon), EditorStyles.label, new GUILayoutOption[2] { GUILayout.Width(iconsSize), GUILayout.Height(iconsSize) })) { }
            GUILayout.Label(new GUIContent(" "), GUILayout.Width(1));
        ...
        {
            FoldHeaderStart(ref foldout, new GUIContent(title), FGUIResources.FoldStyle, style, icon, height);
        }
        ...
        public static void FoldHeaderStart(ref bool foldout, GUIContent title, GUIStyle textStyle, GUIStyle vertStyle, Texture icon = null, int height = 22)
        {
            ...
            if (GUILayout.Button(new GUIContent("   " + FGUIResources.GetFoldSimbol(foldout, 10, "►") + "  " + title.text, icon, title.tooltip), textStyle, GUILayout.Height(height))) foldout = !foldout;
        }
        ...
        public static void FoldSwitchableHeaderStart(ref bool enable, ref bool foldout, string title, GUIStyle style = null, Texture icon = null, int height = 22, string tooltip = "", bool big = false)
        {
            ...
            if (enable)
            {
                if (GUILayout.Button(new GUIContent("  " + FGUIResources.GetFoldSimbol(foldout, 10, "►") + "  " + title, icon, tooltip), big ? FGUIResources.FoldStyleBig : FGUIResources.FoldStyle, GUILayout.Height(height))) foldout = !foldout;
                enable = EditorGUILayout.Toggle(enable, GUILayout.Width(16));
            }
            else
            {
                if (GUILayout.Button(new GUIContent("  " + title, icon, tooltip), big ? FGUIResources.FoldStyleBig : FGUIResources.FoldStyle, GUILayout.Height(height))) { enable = true; }
                enable = EditorGUILayout.Toggle(enable, GUILayout.Width(16));
            }
        ...
        public static void DrawSwitchButton(ref bool switcher, Texture icon, Texture pressedIcon = null, string tooltip = "", int width = 20, int height = 20, bool reversePress = false)
        {
            ...
            {
                if (GUILayout.Button(new GUIContent(pressedIcon, tooltip), FGUIResources.ButtonStyle, new GUILayoutOption[2] { GUILayout.Width(width), GUILayout.Height(height) })) switcher = !switcher;
            }
            else
                if (GUILayout.Button(new GUIContent(icon, tooltip), FGUIResources.ButtonStyle, new GUILayoutOption[2] { GUILayout.Width(width), GUILayout.Height(height) })) switcher = !switcher;
