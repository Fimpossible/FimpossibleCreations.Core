...
namespace FIMSpace.AnimationTools
{
    ...
    /// </summary>
    public static class BodySkeletonFinder
    {
        ...
        public class SkeletonInfo
        {
            ...
            public SkeletonInfo(Transform t, List<Transform> checkOnly = null, Transform pelvisHelp = null)
            {
                ...
                for (int c = 0; c < childT.Count; c++)
                {
                    ...
                    {
                        Transform getCh = BodyBonesFinder.GetBottomMostChildTransform(ct);
                ...
                if (ProbablyHead != null)
                    ...
                    {
                        if (BodyBonesFinder.IsChildOf(ProbablyHead, ProbablyHips) == false)
                        {
                            ProbablyHead = null;
                        }
                    }
                ...
                if (ProbablyChest == null || ProbablyChest == ProbablyHips || (ProbablyHips != null && BodyBonesFinder.IsChildOf(ProbablyChest, ProbablyHips) == false))
                {
                            ...
                            while (checkT.parent != null && checkT.parent != ProbablyHips)
                            {
                                if (checkT.childCount > 2)
                                {
                                    ...
                                    for (int s = 0; s < TrReachingSides.Count; s++)
                                    {
                                        ...
                                        if (BodyBonesFinder.IsChildOf(side, checkT))
                                        {
                                            found = true;
                                            break;
                                        }
                ...
                if (ProbablyChest && ProbablyHips)
                {
                    ...
                    if (ProbablyHead)
                    {
                        ...
                        {
                            if (BodyBonesFinder.IsChildOf(TrReachingSides[i], ProbablyHead)) TrReachingSides.RemoveAt(i);
                        }
                    }

                    for (int i = TrReachingSides.Count - 1; i >= 0; i--)
                    {
                        if (BodyBonesFinder.GetDepth(TrReachingSides[i], AnimatorTransform) < 5)
                        {
                    ...
                    #region Checking Bones existence in other arms to remove duplicates

                    CheckToRemoveDuplicates(ProbablyLeftArms, ProbablyLeftLegs);
                    CheckToRemoveDuplicates(ProbablyRightArms, ProbablyRightLegs);

                    CheckToRemoveDuplicates(ProbablyLeftLegs, ProbablyLeftArms);
                    CheckToRemoveDuplicates(ProbablyRightLegs, ProbablyRightArms);

                    // Extra spine bones error cleanup
                    List<Transform> extras = new List<Transform>() { ProbablyHips, ProbablyRootBone, t, AnimatorTransform };
                    CheckForExtraCleanup(ProbablyLeftArms, ProbablySpineChain, extras);
                    CheckForExtraCleanup(ProbablyRightArms, ProbablySpineChain, extras);

                    CheckForExtraCleanup(ProbablyLeftLegs, ProbablySpineChain, extras);
                    CheckForExtraCleanup(ProbablyRightLegs, ProbablySpineChain, extras);

                    // Try recontruct lost chains
                    for (int i = 0; i < ProbablyLeftArms.Count; i++) TryReconstructTo(ProbablyLeftArms[i], 3);
                    for (int i = 0; i < ProbablyRightArms.Count; i++) TryReconstructTo(ProbablyRightArms[i], 3);
                    for (int i = 0; i < ProbablyLeftLegs.Count; i++) TryReconstructTo(ProbablyLeftLegs[i], 3);
                    for (int i = 0; i < ProbablyRightLegs.Count; i++) TryReconstructTo(ProbablyRightLegs[i], 3);
            ...
            void CheckToRemoveDuplicates(List<List<Transform>> selfList, List<List<Transform>> otherList)
            {
                for (int a = selfList.Count - 1; a >= 0; a--)
                {
                    var mainArm = selfList[a];
                    for (int o = 0; o < selfList.Count; o++)
                    {
                        if (a == o) continue;

                        for (int b = 0; b < mainArm.Count; b++)
                        {
                            if (selfList[o].Contains(mainArm[b]))
                            {
                                selfList.RemoveAt(o);
                                break;
                            }
                ...
                if ( otherList != null)
                {
                    ...
                    {
                        var mainArm = selfList[a];
                        for (int o = 0; o < otherList.Count; o++)
                        {
                            if (a == o) continue;

                            for (int b = 0; b < mainArm.Count; b++)
                            {
                                if (otherList[o].Contains(mainArm[b]))
                                {
            ...
            void CheckForExtraCleanup(List<List<Transform>> limbs, List<Transform> extraCleanup, List<Transform> extraBones)
            {
                for (int i = 0; i < limbs.Count; i++)
                {
                    for (int s = 0; s < extraBones.Count; s++) if (extraBones[s]) if (limbs[i].Contains(extraBones[s])) { limbs[i].Remove(extraBones[s]); }
                    for (int s = 0; s < extraCleanup.Count; s++) if (extraCleanup[s]) if (limbs[i].Contains(extraCleanup[s])) { limbs[i].Remove(extraCleanup[s]); }
                }

                for (int i = limbs.Count - 1; i >= 0; i--)
                {
                    if (limbs[i].Count == 0) limbs.RemoveAt(i);
                }
            }

            void TryReconstructTo(List<Transform> list, int targetCount)
            {
                ...
                {
                    Transform newBone = BodyBonesFinder.GetContinousChildTransform(list[i-1]);
                    if (newBone) list.Add(newBone);
                }
            }
        ...
        /// <summary> Good for hips / head search using naming </summary>
        public static Transform TryFind(Transform baseTransform, string[] names, Animator animator = null, bool getFarestIfNotSure = true)
        {
            if (baseTransform == null) return null;

            if (animator != null)
            {
                if (animator.isHuman) return animator.GetBoneTransform(HumanBodyBones.Hips);
            }

            var allChild = baseTransform.GetComponentsInChildren<Transform>();
            List<Transform> probablyHips = new List<Transform>();

            for (int c = 0; c < allChild.Length; c++)
            {
                if (allChild[c].GetComponent<Renderer>()) continue; // Its not a bone - its some kind of mesh!

                if (NameContains(allChild[c].name, names))
                {
                    probablyHips.Add(allChild[c]);
                }
            }

            if (probablyHips.Count == 0) return null;
            if (probablyHips.Count == 1) return probablyHips[0];

            if (getFarestIfNotSure == false) return probablyHips[0];

            float farest = 0f;
            Transform farestT = probablyHips[0];
            for (int i = 0; i < probablyHips.Count; i++)
            {
                float distance = Vector3.Distance(baseTransform.position, probablyHips[i].position);
                if (distance > farest)
                {
                    farest = distance;
                    farestT = probablyHips[i];
                }
            }
        ...
        /// <summary> Utility method </summary>
